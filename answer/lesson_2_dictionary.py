...
class dictionary_practice:
    ...
    def dict_add(self, uav_name, drone_id, drone):
        ...
        uav = dict()
        uav["drone_id"] = drone_id
        uav["drone"] = drone
        self.group[uav_name] = uav
    ...
    def dict_modify(self, uav_name, new_drone_id, new_velocity_x, new_velocity_y, new_velocity_z):
        ...
        # 注意思考 如何修改uav的member
        self.group[uav_name]["drone_id"] = new_drone_id
        self.group[uav_name]["drone"].velocity_x = new_velocity_x
        self.group[uav_name]["drone"].velocity_y = new_velocity_y
        self.group[uav_name]["drone"].velocity_z = new_velocity_z

    def dict_del(self, uav_name):
        # 給你需要刪除的uav_name, 把它從group裡面刪除,
        # 如果刪除成功, return True
        # 如果原本uav_name並不存在group裡面, return None
        feedback = self.group.pop(uav_name, None)
        if feedback:
            feedback = True
        return feedback
